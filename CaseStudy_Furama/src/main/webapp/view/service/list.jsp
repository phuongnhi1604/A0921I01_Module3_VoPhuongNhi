    ...
    <div class="row" style="min-height: 100%">
        <div class="col-md-2 mt-3 bg-gradient" style="background-color: #FADA5C">
            <ul class="nav flex-column" >
                <li class="nav-item">
                    <a class="nav-link a1-color fs-5" href="/services?action=add">Add new service</a>
                </li>
                <li class="nav-item">
        ...
        <div class="col-md-9 mt-3" style="background-color: #FDF9E4">
            <table class="table table-warning table-striped border-danger" border="1" style="border-collapse:  collapse">
                ...
                <c:forEach items="${serviceList}" var="servcie" varStatus="item">
                    <tr>
                        <td>${item.count}</td>
                        <td>${servcie.getId()}</td>
                        <td>${servcie.getService_name()}</td>
                        <td>${servcie.getService_area()}</td>
                        <td>${servcie.getService_max_people()}</td>

                        <c:forEach items="${rentTypeList}" var="rentType">
                            <c:if test="${rentType.getRent_type_id() == servcie.getRent_type_id()}">
                                <td>${rentType.getRent_type_name()}</td>
                            </c:if>
                        </c:forEach>

                        <c:forEach items="${serviceTypeList}" var="serviceType">
                            <c:if test="${serviceType.getService_type_id() == servcie.getService_type_id()}">
                                <td>${serviceType.getService_type_name()}</td>
                            </c:if>
                        </c:forEach>

                        <td>${servcie.getStandard_room()}</td>
                        <td>${servcie.getDescription_other_convenience()}</td>
                        <td>${servcie.getPool_area()}</td>
                        <td>${servcie.getNumber_of_floors()}</td>
                    </tr>
                </c:forEach>
            </table>
        </div>
